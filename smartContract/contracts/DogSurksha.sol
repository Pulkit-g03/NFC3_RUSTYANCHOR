...
import "./interfaces/IClaimDogSuraksha.sol";
...
contract DogSurksha {
    ...
    IClaimDogSuraksha private claimDogSuraksha;
    ...
    function approveClaim(bytes32 policyId) external onlyAdmin returns (bool) {
        ...
        (, , , , bool isAdminApproved) = claimDogSuraksha
        ...
        claimDogSuraksha.updateClaimStatus(policyId, true);
    ...
    modifier onlyClaimDogSurakshaContract() {
        require(
            msg.sender == address(claimDogSuraksha),
            "Only DogSurakshaClaimContract can perform this action"
        );
        _;
    }
    ...
    ) external onlyClaimDogSurakshaContract {
        Policy storage _policy = policy[policyId];
